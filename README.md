...
```bash
node /Users/gimdong-wan/Codex-Claude-Debate/src/app/cli.mjs
```
...
node /Users/gimdong-wan/Codex-Claude-Debate/src/app/cli.mjs "사용자 인증 플로우를 리팩터링하고 테스트까지 맞춰줘"
```
...
- 시작 시 `codex`/`claude` 인증을 먼저 확인합니다.
- 채팅 UI의 `tty output` suspend 문제를 줄이기 위한 신호 처리 보완이 들어갔습니다.
- planning은 이제 최종 단계에서 Codex 안을 자동 채택하지 않습니다.
- Codex와 Claude가 각자 final plan 초안을 만들고, 서로 리뷰한 뒤, 실질적으로 같은 plan에 수렴했을 때만 shared plan을 채택합니다.

다음 작업자가 먼저 확인할 권장 순서:

1. `npm run doctor`
2. `zsh run-debate.sh`
3. 필요하면 `node src/app/cli.mjs plan "간단한 작업"`으로 finalization 출력 확인
